  ...
  list.forEach(item => {
    const row = html`<div class="list-item">
      <div>
        ...
        ${item.url
          ? html`<div class="small"><a target="_blank" href="${item.url}">${item.url}</a></div>`
          : ""}
...
export async function HomePage() {
  ...
  node.getElementById("bm-add").addEventListener("click", async () => {
    ...
    node.getElementById("bm-title").value = "";
    node.getElementById("bm-url").value = "";
    node.getElementById("bm-notes").value = "";

    await MerkenList(node.getElementById("bm-list"));
  });

  // Datei-Upload
  node.getElementById("upload").addEventListener("click", async () => {
    const f = node.getElementById("file").files?.[0];
    if (!f) return alert("Bitte eine Datei wählen");

    const path = await uploadFile(f);
    const url = await signedUrl(path);

    node.getElementById("file-link").innerHTML =
      `Link (1h gültig): <a target="_blank" href="${url}">Öffnen</a>`;
  });

  // Seite mounten und Bookmark-Liste laden
